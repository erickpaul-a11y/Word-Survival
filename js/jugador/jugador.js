class Jugador {
    constructor(){
        ...
        this.piernas=[];
        [-.17,.17].forEach(x=>{const p=new THREE.Mesh(new THREE.BoxGeometry(.22,.72,.25),pantalon);p.position.set(x,.28,0);p.castShadow=true;this.modelo.add(p);this.piernas.push(p);});
        this.piernas.forEach((p,i)=>{const s=new THREE.Mesh(new THREE.BoxGeometry(.24,.13,.31),zapato);s.position.set(p.position.x,-.08,.025);s.castShadow=true;this.modelo.add(s);});

        this.brazos=[];
        [-.40,.40].forEach(x=>{const b=new THREE.Mesh(new THREE.BoxGeometry(.18,.68,.20),ropa);b.position.set(x,1.08,0);b.castShadow=true;this.modelo.add(b);this.brazos.push(b);});
        ...
        const boca=new THREE.Mesh(new THREE.BoxGeometry(.13,.035,.025),negro); boca.position.set(0,1.62,.326); this.modelo.add(boca);
    ...
    agregarAEscena(e){
        ...
        e.add(this.modelo);
        this.modelo.visible=true;
        if(window.motor){window.motor.cameraMode='first';window.motor.alturaCamara=0.88;}
    ...
    actualizarManos(dt){
        ...
        this._golpeL=Math.max(0,this._golpeL-dt*5); this._golpeR=Math.max(0,this._golpeR-dt*5);
    ...
    actualizarPosicion(){
        ...
        if(window.motor){
            this.modelo.rotation.y=window.motor.ang.y;
        }
        this.animarMovimiento(.016);
    }

    animarMovimiento(dt=.016){
        const a=this.modelo.userData.animacion;if(!a)return;
        const mov=this.velocidadMovimiento>.08;
        if(mov)this._tiempoAnimacion+=dt*(this.corriendo?13:9);
        const amplitud=mov?(this.corriendo?.62:.42):0;
        const paso=Math.sin(this._tiempoAnimacion)*amplitud;
        a.piernas.forEach((p,i)=>p.rotation.x=paso*(i%2?-1:1));
        a.brazos.forEach((b,i)=>b.rotation.x=paso*(i%2?1:-1));
    }

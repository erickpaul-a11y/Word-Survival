...
/*
 * Las texturas de criaturas y piel se generan como pequeñas superficies
 * pintadas y repetibles. Así no se pega la imagen completa de una vaca
 * sobre cada pata/cabeza: cada pieza recibe un material coherente.
 */
const ARCHIVOS={
...
function texturaPintada(tipo){
    ...
    /* Base pintada: manchas suaves y pinceladas grandes. */
    for(let i=0;i<34;i++){
        const px=(i*73+19)%256, py=(i*131+37)%256;
        const rx=7+(i*17)%38, ry=4+(i*11)%24;
        x.globalAlpha=.10+(i%6)*.025;
        x.fillStyle=pal[1+(i%Math.min(3,pal.length-1))];
        x.beginPath();
        x.ellipse(px,py,rx,ry,(i%11)*.19,0,Math.PI*2);
        x.fill();
    }

    /* Pinceladas especiales para que cada material tenga identidad. */
    if(tipo==='vaca'){
        x.globalAlpha=.72;
        x.fillStyle=pal[2];
        [[48,62,30,21],[166,48,38,27],[116,157,43,30],[216,202,27,19]].forEach(a=>{
            x.beginPath(); x.ellipse(a[0],a[1],a[2],a[3],.18,0,Math.PI*2); x.fill();
        });
    }else if(tipo==='oveja'){
        x.globalAlpha=.35; x.fillStyle=pal[2];
        for(let i=0;i<22;i++){
            x.beginPath(); x.arc((i*47)%256,(i*83)%256,10+(i%4)*3,0,Math.PI*2); x.fill();
        }
    }else if(tipo==='pollo'){
        x.globalAlpha=.42; x.strokeStyle=pal[2]; x.lineWidth=7;
        for(let i=0;i<18;i++){
            x.beginPath(); x.moveTo((i*31)%256,20+(i*17)%236); x.lineTo((i*31+14)%256,28+(i*17)%236); x.stroke();
        }
    }else if(tipo==='pez'){
        x.globalAlpha=.38; x.strokeStyle=pal[2]; x.lineWidth=3;
        for(let i=0;i<9;i++){
            x.beginPath(); x.arc(30+i*28,128,70,Math.PI*.12,Math.PI*.88); x.stroke();
        }
    }else if(tipo==='humano'){
        x.globalAlpha=.20; x.fillStyle=pal[3];
        for(let i=0;i<12;i++){
            x.beginPath(); x.ellipse(20+i*23,(i*41)%256,22,8,.2,0,Math.PI*2); x.fill();
        }
    }
...
function tipoLocal(n){
    ...
    if(raw==='ropa')return'ropa';
    if(raw==='pantalon')return'pantalon';
    if(raw==='pelo'||raw==='cabello')return'pelo';
    if(raw==='zapato')return'zapato';
    if(raw==='ojo')return'ojo';
    if(raw==='boca')return'boca';
...
function asegurarUV(g){
    ...
    for(let i=0;i<p.count;i++){
        const k=i*3,px=p.array[k],pz=p.array[k+2];
    ...
    for(let i=0;i<p.count;i++){
        const k=i*3;
        uv.push((p.array[k]-minX)/dx,(p.array[k+2]-minZ)/dz);
    }
...
function optimizarGeometria(n,tipo){
    ...
    if(tipo==='hojas'&&n.geometry.type==='SphereGeometry'){
        const old=n.geometry;
        const r=old.parameters&&old.parameters.radius?old.parameters.radius:1;
        n.geometry=new THREE.IcosahedronGeometry(r,1);
        old.dispose();
    }
    /* Pequeños detalles mantienen pocos polígonos para no romper el estilo. */
    if(n.geometry.type==='SphereGeometry'&&n.geometry.parameters){
        const p=n.geometry.parameters;
        if(p.widthSegments>8||p.heightSegments>6){
            const old=n.geometry;
            n.geometry=new THREE.SphereGeometry(p.radius||1,8,6);
            old.dispose();
        }
    }
...
function animarAgua(t){
    for(let i=aguas.length-1;i>=0;i--){
        ...
        for(let j=0;j<pos.count;j++){
            const k=j*3,xx=base[k],zz=base[k+2];
            pos.array[k+1]=base[k+1]
                +Math.sin(xx*.72+t*1.55+fase)*.035
                +Math.cos(zz*.58+t*1.2+fase)*.028
                +Math.sin((xx+zz)*.32+t*.75+fase)*.018;
        }

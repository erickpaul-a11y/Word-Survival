...
(function() {
  ...
  // Redefinición de romper/excavar terreno 3D
  Mundo.prototype.destruirBloque = function(hit, radio) {
    if (!hit || !hit.point) return null;
    const p = hit.point;
    const r = radio || 1.5;

    // Reducir la densidad volumétrica en la posición del impacto
    this.excavar(p, r);

    return { x: p.x, y: p.y, z: p.z, radio: r };
  };

  // Colocar o rellenar masa de tierra en un punto tridimensional
  Mundo.prototype.colocarBloque = function(x, y, z, radio) {
    const pos = new THREE.Vector3(x, y, z);
    const r = radio || 1.5;

    // Incrementar la densidad en la coordenada 3D
    this.anadirTierra(pos, r);

    return { x, y, z, radio: r };
  };

  // Colocar tierra usando el impacto del Raycaster
  Mundo.prototype.colocarTierraCercana = function(hit, radio) {
    if (!hit || !hit.point) return null;
    
    let pos = hit.point.clone();
    if (hit.face && hit.face.normal) {
      // Ajuste usando la normal de la cara para proyectar el bloque hacia afuera

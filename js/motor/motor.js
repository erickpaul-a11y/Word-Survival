class Motor {
    constructor(){
        this.escena=new THREE.Scene();this.cam=new THREE.PerspectiveCamera(75,window.innerWidth/window.innerHeight,.1,1000);this.renderer=new THREE.WebGLRenderer({antialias:true});this.renderer.setSize(window.innerWidth,window.innerHeight);this.renderer.domElement.id='canvas-3d';document.body.appendChild(this.renderer.domElement);
        this.ang={x:0,y:0};this.teclas={};this.cameraMode='first';this.j=null;this.mundo=null;this.criaturas=null;this.inv=null;this.crafteos=null;this.manoIzquierda=false;this.ultimoGolpe=0;
        ...
        window.addEventListener('keydown',e=>{const k=e.key.toLowerCase();this.teclas[k]=true;if(['w','a','s','d',' ','shift','arrowup','arrowdown','arrowleft','arrowright'].includes(k))e.preventDefault();if(k===' '&&!e.repeat)this.saltar();if(k==='e'&&!e.repeat)this.agarrar();if(k==='c'&&!e.repeat&&this.crafteos)this.crafteos.toggle();});
        window.addEventListener('keyup',e=>{this.teclas[e.key.toLowerCase()]=false;});
        this.renderer.domElement.addEventListener('mousedown',e=>{if(e.button!==0)return;this.manoIzquierda=true;if(this.j)this.j.actualizarManoIzquierda(this.cam,true);this.golpearRecurso();});
        window.addEventListener('mouseup',e=>{if(e.button!==0)return;this.manoIzquierda=false;if(this.j)this.j.actualizarManoIzquierda(this.cam,false);});
        ...
        document.addEventListener('mousemove',e=>{if(document.pointerLockElement!==this.renderer.domElement)return;this.ang.y-=e.movementX*.0025;this.ang.x-=e.movementY*.0025;const l=Math.PI/2-.05;this.ang.x=Math.max(-l,Math.min(l,this.ang.x));if(this.j&&this.manoIzquierda)this.j.actualizarManoIzquierda(this.cam,true);});
    ...
    iniciar(){
        try{if(typeof Jugador!=='undefined'){this.j=new Jugador();this.j.y=this.getGroundHeightAt(0,0)+this.alturaJugador;this.j.agregarAEscena(this.escena);this.j.actualizarPosicion();}}catch(e){console.warn('Error creando jugador:',e);}
        try{if(typeof Mundo!=='undefined')this.mundo=new Mundo(this);}catch(e){console.warn('Error creando mundo:',e);}
        try{const datos={vaca:{vida:20,velocidad:.014},oveja:{vida:15,velocidad:.012},pollo:{vida:8,velocidad:.018}};if(typeof GestorCriaturas!=='undefined')this.criaturas=new GestorCriaturas(this,datos);if(this.criaturas)this.criaturas.generarIniciales();}catch(e){console.warn('Error creando criaturas:',e);}
        this._actualizarCamara();const loop=()=>{this._updateMovement();try{if(this.j&&this.mundo)this.mundo.actualizar(this.j.x,this.j.z);}catch(e){}try{if(this.criaturas)this.criaturas.actualizar();}catch(e){}if(this.j&&this.manoIzquierda)this.j.actualizarManoIzquierda(this.cam,true);if(this.j)this.j.actualizarHUD();this._actualizarCamara();this.renderer.render(this.escena,this.cam);requestAnimationFrame(loop);};requestAnimationFrame(loop);
    }
    saltar(){if(!this.j||!this.enSuelo)return;this.velocidadY=this.fuerzaSalto;this.enSuelo=false;}
    _updateMovement(){if(!this.j)return;const f=(this.teclas.w?1:0)-(this.teclas.s?1:0),r=(this.teclas.d?1:0)-(this.teclas.a?1:0);let dx=0,dz=0;if(f||r){const y=this.ang.y;dx=Math.cos(y)*f-Math.sin(y)*r;dz=Math.sin(y)*f+Math.cos(y)*r;const l=Math.hypot(dx,dz);if(l){dx/=l;dz/=l;}const v=this.teclas.shift?this.velocidadCorrer:this.velocidadCaminar;this.j.x+=dx*v;this.j.z+=dz*v;}this.velocidadY-=this.gravedad;this.j.y+=this.velocidadY;const suelo=this.getGroundHeightAt(this.j.x,this.j.z)+this.alturaJugador;if(this.j.y<=suelo){this.j.y=suelo;this.velocidadY=0;this.enSuelo=true;}else this.enSuelo=false;if(this.j.actualizarPosicion)this.j.actualizarPosicion();}
    ...
    golpearRecurso(){const ahora=performance.now();if(ahora-this.ultimoGolpe<350)return;this.ultimoGolpe=ahora;const o=this.encontrarObjetivo();if(!o||!o.userData.recurso)return;const r=o.userData.recurso;if(this.inv){Object.entries(r).forEach(([id,qty])=>this.inv.agregar(id,qty,id));}if(o.parent)o.parent.remove(o);console.log('Recurso obtenido:',r);}

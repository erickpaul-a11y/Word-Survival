class Motor {
    ...
    golpearRecurso(){const ahora=performance.now();if(ahora-this.ultimoGolpe<350)return;this.ultimoGolpe=ahora;const hit=this.encontrarHit();let o=hit&&hit.object;while(o){if(o.userData&&o.userData.recurso)break;o=o.parent;}if(o&&o.userData.recurso){const r=o.userData.recurso;if(this.inv)Object.entries(r).forEach(([id,qty])=>this.inv.agregar(id,qty,id));if(o.parent)o.parent.remove(o);return;}if(!this.inv||!this.inv.items.pala_madera){this.mostrarMensaje('Necesitas una pala de madera para excavar.');return;}const bloque=this.mundo&&this.mundo.destruirBloque(hit);if(bloque){this.inv.agregar('tierra',1,'tierra');const veta=this.mundo.ruidoSuave(bloque.x*19.7,bloque.z*31.1);if(veta<.38)this.inv.agregar('piedra',1,'piedra encontrada bajo tierra');}}

class Motor {
    ...
    _updateMovement(dt){
        ...
        if(this.enAgua&&water!==null){const controlVertical=(this.teclas[' ']?1:0)-(this.teclas.control?1:0);this.velocidadY+=(water+this.alturaJugador*.35-this.j.y)*7*dt+controlVertical*9*dt;this.velocidadY*=Math.pow(.16,dt);this.j.y+=this.velocidadY*dt;this.j.y=Math.max(this.getGroundHeightAt(this.j.x,this.j.z)+.18,this.j.y);this.enSuelo=false;this.j.saltando=false;}
    ...
    crearObjeto(id,nombre,x,z,qty=1,yOverride=null){
        const colores={madera:0x8b5a2b,palo:0xc48a4a,piedra:0x8d99a6,tierra:0x6e3b1c,pala_madera:0xc48a4a,pico_piedra:0x8d99a6};
        let geo;
        if(id==='palo'){
            geo=new THREE.CylinderGeometry(.055,.07,.72,8);
        }else if(id==='madera'){
            geo=new THREE.CylinderGeometry(.16,.20,.48,8);
        }else if(id==='tierra'){
            geo=new THREE.BoxGeometry(.32,.32,.32);
        }else{
            geo=new THREE.DodecahedronGeometry(.18+(id==='madera'?.12:0),0);
        }
        const m=new THREE.Mesh(geo,new THREE.MeshLambertMaterial({color:colores[id]||0xffd166}));m.castShadow=true;m.receiveShadow=true;
        m.position.set(x,yOverride===null?this.getGroundHeightAt(x,z)+.25:yOverride,z);
        m.rotation.set(Math.random()*2,Math.random()*2,0);m.userData.pickup={id,name:nombre||id,qty:1};m.userData.velY=.8+Math.random()*1.4;m.userData.suelo=true;
        this.escena.add(m);this.objetosSuelo.push(m);return m;
    }
    soltarRecursos(recursos,pos){
        const entrada=Object.entries(recursos||{});
        if(!entrada.length)return;
        const [id,qty]=entrada[0];
        if(qty>0){this.crearObjeto(id,id,pos.x,pos.z,1);}
    }
    ...
    equiparCercano(){const hit=this.encontrarHit();let o=hit&&hit.object;while(o&&!o.userData.pickup)o=o.parent;if(!o){o=this.objetosCercanos(2.5).sort((a,b)=>a.position.distanceTo(this.cam.position)-b.position.distanceTo(this.cam.position))[0];}if(!o){this.desequipar();return;}if(this.equipado){this.mostrarMensaje('Ya tienes un objeto en la mano.');return;}this.retirarObjeto(o);this.equipado=o;this.j.rightHand.add(o);o.position.set(.02,-.24,-.2);o.scale.setScalar(1.25);this.mostrarMensaje(`En mano: ${o.userData.pickup.name}`);}
    desequipar(){if(!this.equipado)return;const o=this.equipado;this.equipado=null;const p=this.j.rightHand.getWorldPosition(new THREE.Vector3());this.crearObjeto(o.userData.pickup.id,o.userData.pickup.name,p.x,p.z,1,p.y-.08);this.j.rightHand.remove(o);}
    ...
    golpearRecurso(){
        const ahora=performance.now();if(ahora-this.ultimoGolpe<350)return;this.ultimoGolpe=ahora;
        const hit=this.encontrarHit();let pickup=hit&&hit.object;while(pickup&&!pickup.userData.pickup)pickup=pickup.parent;
        if(pickup&&pickup.userData.suelo){this.moverObjeto(1);return;}
        let o=hit&&hit.object;while(o&&!(o.userData&&o.userData.recurso))o=o.parent;if(o&&o.parent&&o.parent.userData.arbol)o=o.parent;
        if(o&&o.userData.recurso){
            if(o.userData.arbol){
                o.userData.vida--;
                if(o.userData.vida<=0)this.mundo.derribarArbol(o);
                else this.crearObjeto('madera','madera',o.position.x,o.position.z,1);
                return;
            }
            const recursos=o.userData.recurso;const primero=Object.keys(recursos)[0];if(primero){this.crearObjeto(primero,primero,o.getWorldPosition(new THREE.Vector3()).x,o.getWorldPosition(new THREE.Vector3()).z,1);}
            return;
        }
        if(!this.equipado||this.equipado.userData.pickup.id!=='pala_madera'){this.mostrarMensaje('Equipa una pala de madera con E para excavar.');return;}
        if(!hit||!hit.object||!hit.object.userData.interactivo){this.mostrarMensaje('Apunta a la tierra para excavar.');return;}
        const bloque=this.mundo&&this.mundo.destruirBloque(hit);
        if(bloque){
            const tierraY=bloque.y+.65;
            this.crearObjeto('tierra','tierra',bloque.x,bloque.z,1,tierraY);
            this.mostrarMensaje('Tierra excavada: quedó arriba del agujero.');
        }
    }
